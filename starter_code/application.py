...
def helper_function(log: Dict[str, List[Dict]], src_num: str, dst_num: str,
                    call: Call, customer_list: List[Customer]) -> None:
    """
    Checks if a customer has a specific Phoneline, and then makes or
    receives the call.
    """
    for customer_data in log['customers']:
        for line in customer_data['lines']:
            if line['number'] == src_num:
                customer11 = find_customer_by_number(line['number'],
                                                     customer_list)
                customer11.make_call(call)
            elif line['number'] == dst_num:
                customer11 = find_customer_by_number(
                    line['number'], customer_list)
                customer11.receive_call(call)
...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        current_month = datetime.datetime.strptime(event_data['time'],
                                                   "%Y-%m-%d %H:%M:%S").month
        ...
        if event_data['type'] == 'call':
            ...
            call = Call(src_num, dst_num,
                        datetime.datetime.strptime(event_data['time'],
                                                   "%Y-%m-%d %H:%M:%S"),
                        int(event_data['duration']),
                        event_data['src_loc'],
                        event_data['dst_loc'])
            helper_function(log, src_num, dst_num, call, customer_list)
