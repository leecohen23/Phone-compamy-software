    ...
    def apply(self, customers: List[Customer],
        ...
        for cust in customers:
            ...
            try:
                ...
                return data
            if k == int(filter_string):
                history = cust.get_history()
                for q in data:
                    for t in history[0]:
                        ans.append(t)# if t == q else None)
                    for r in history[1]:
                        ans.append(r) #if r == q else None)
                data = list(set(data).intersection(ans))
        return ans
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if filter_string[0] == 'L':
            for calls in data:
                if calls.duration < int(seconds):
                    a.append(calls)
        elif filter_string[0] == 'G':
            for calls in data:
                if calls.duration > int(seconds):
                    a.append(calls)
        data = list(set(data).intersection(a))
        return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if not filter_string:
            return data
        filter1 = filter_string.replace(',', ', ').replace('  ', ' ').split(
            ', ')
        a = []
        for k in filter1:
            try:
        ...
        for calls in data:
            ...
            if a[0] > a[2] and a[1] > a[3]:
                if qq < a[0] or -79.697878 > a[2] or a[1] > 43.799568 or \
                        a[3] < 43.576959:
                    return data
                if (a[0] >= x1 >= a[2] and a[1] >= y1 >= a[3]) or (
                        a[0] >= x2 >= a[2] and a[1] >= y2 >= a[3]):
                    ans.append(calls)
            elif a[0] > a[2] and a[1] < a[3]:
                if qq < a[0] or -79.697878 > a[2] or a[3] > 43.799568 or \
                        a[1] < 43.576959:
                    return data
                if (a[0] >= x1 >= a[2] and a[3] >= y1 >= a[1]) or (
                        a[0] >= x2 >= a[2] and a[3] >= y2 >= a[1]):
                    ans.append(calls)
            elif a[0] < a[2] and a[1] > a[3]:
                if qq < a[2] or -79.697878 > a[0] or a[1] > 43.799568 or \
                        a[3] < 43.576959:
                    return data
                if (a[2] >= x1 >= a[0] and a[1] >= y1 >= a[3]) or (
                        a[2] >= x2 >= a[0] and a[1] >= y2 >= a[3]):
                    ans.append(calls)
            elif a[0] < a[2] and a[1] < a[3]:
                if qq < a[2] or -79.697878 > a[0] or a[3] > 43.799568 or \
                        a[1] < 43.576959:
                    return data
                if (a[2] >= x1 >= a[0] and a[3] >= y1 >= a[1]) or (
                        a[2] >= x2 >= a[0] and a[3] >= y2 >= a[1]):
                    ans.append(calls)
            else:
                return data
        data = list(set(data).intersection(ans))
